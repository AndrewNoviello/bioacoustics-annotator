# Modifications by Manolo 5/2025:
# line 34 added a dynamic gain function for spectrogram generation using an offset based on IQR clamped to limits 
# line 201 updated generate spectrogram results to consider the dynamic gain function (and plottign in line 219)
# line 406 updated generate segments to consider the dynamic gain function (removed dBrange=90 from lines 158 & 364)
# This way now the script will consider the power (dB) percentile range from the entire soundfile or segment to adjust the dB gain and
# the offset will be based on signal contrast within predefined lower and upper bounds
...
def calculate_dynamic_db_gain(S):
    ...
    # Calculate adaptive offset from IQR
    offset = 10 * np.log10(1 / (iqr + 1e-10))

    # Clamp offset to a reasonable range
    offset = max(min(offset, 10), -10) # tweak these upper/lower boudns as needed

    # Compute median-based gain
    median_power = np.median(S)
    db_gain = 10 * np.log10(1 / (median_power + 1e-10)) + offset
...
def generate_spectrogram_results(wav, sr, seg_size=None, preds=None, total_scores=None,
    ...
    wav, sr, nperseg=nperseg, noverlap=noverlap,
    flims=[khz_lims[0] * 1000, khz_lims[1] * 1000]
    )
    ext[-1] = khz_lims[1]

    # Compute initial db_gain
    db_gain = calculate_dynamic_db_gain(Sxx)
    db_gain = max(min(db_gain, 60), 10)  # #tweak this upper/lower bounds as needed

    # Convert to dB with wide range for percentile estimation
    Sxx_db_temp = util.power2dB(Sxx, db_gain=db_gain, db_range=200)

    # Compute adaptive db_range from percentiles
    adaptive_db_range = np.percentile(Sxx_db_temp, 99.9) - np.percentile(Sxx_db_temp, 0.1)

    # Clamp db_range to avoid overly small or overly large ranges
    adaptive_db_range = max(min(adaptive_db_range, 100), 20) #tweak this upper/lower bounds as needed

    # Final conversion using safe adaptive db_range
    Sxx_db = util.power2dB(Sxx, db_gain=db_gain, db_range=adaptive_db_range)

    # Plot
    fig, ax = plt.subplots(figsize=(30, 5)) #image size of segment spectrogram
    ax.imshow(Sxx_db, extent=ext, origin="lower", cmap=cmap, 
          vmin=-adaptive_db_range, vmax=0)
    ax.axis("tight")
    fig.tight_layout()
...
def batch_audio_detection(wav_list, neg_prompts, pos_prompts, theta, save_path):
    ...
    with open(save_path, 'w') as det_preds:
        det_preds.write("id,filename,start_time,end_time,species,detection_conf,verified\n")
        for f, st, p, s in zip(inf_dset.data, inf_dset.sts, preds, total_scores):
            if p == 1:
                det_preds.write("{},{},{},{},{},{},{}\n".format(str(uuid.uuid4()), f, st, st+inf_dset.seg_size, "null", s[1], 0))
...
def generate_segs(wav_path, file_ann, 
    ...
    - file_ann (pandas.DataFrame): A dataframe containing the annotation data with columns 'start_time',
      'end_time', and 'detection_conf'.
    - save_path (str, optional): The base path where the spectrogram and segments are saved. Defaults to "./temp".
    ...
    db_gain = calculate_dynamic_db_gain(Sxx)
    db_gain = max(min(db_gain, 60), 10)  # Clamp gain between 10 and 60 dB

    # Initial dB conversion with wide range to avoid clipping
    Sxx_db_temp = util.power2dB(Sxx, db_gain=db_gain, db_range=200)

    # Adaptive db_range from percentiles
    adaptive_db_range = np.percentile(Sxx_db_temp, 99.9) - np.percentile(Sxx_db_temp, 0.1)

    # Final dB conversion with adaptive scaling
    Sxx_db = util.power2dB(Sxx, db_gain=db_gain, db_range=adaptive_db_range)

    fig, ax = plt.subplots(figsize=(50, 10)) #image size of full file spectrogram
    ...
    for i in range(len(file_ann)):
        ...
        st = file_ann["start_time"].values[i]
        ed = file_ann["end_time"].values[i]
