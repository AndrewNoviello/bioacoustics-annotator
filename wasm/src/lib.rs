...
fn spectrogram(
    ...
    // Removed overly restrictive assertion about hop_length
    ...
    // Fixed loop bounds - removed unnecessary pad_size subtraction
    for i in (0..=padded_waveform.len().saturating_sub(win_length)).step_by(hop_length) {
...
fn fft(input: Vec<f32>, n_fft: usize) -> Vec<Complex<f32>> {
    ...
    assert!(
        input.len() <= n_fft,
        "input length must be less than or equal to n_fft"
    );

    // Fixed termination condition - should check n_fft, not input length
    if n_fft <= 1 {
        return if input.is_empty() {
            vec![Complex::new(0.0, 0.0)]
        } else {
            vec![Complex::new(input[0], 0.0)]
        };
    }

    // Pad input to n_fft length
    let mut padded_input = input;
    padded_input.resize(n_fft, 0.0);
    ...
    // Fixed recursive FFT calls - use actual lengths, not n_fft/2
...
fn mel_filter_bank(
    ...
    let f_max = f_max.min(f_nyquist);
    
    // Correct frequency calculation for FFT bins
    ...
    let mut fbanks: Vec<Vec<f32>> = up_slopes
        ...
        .map(|(up, down)| {
            let row = down
                .iter()
                .zip(up.iter())
                .map(|(&d, &u)| d.min(u).max(0.0))
                .collect::<Vec<f32>>();
            row
        })
        .collect();

    // Apply Slaney normalization with protection against division by zero
    for i in 0..n_mels {
        let freq_diff = f_points[i + 2] - f_points[i];
        if freq_diff.abs() > 1e-10 {
            let enorm = 2.0 / freq_diff;
